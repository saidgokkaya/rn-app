...
export function PermitForm() {
  ...
  useEffect(() => {
    ...
    const fetchWarehouses = async () => {
      try {
        ...
        const response = await fetch(`${CONFIG.apiUrl}/Ruhsat/warehouses-classes?ruhsatSinifiId=${selectedClassId}`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`
          }
        ...
        toast.error('Depolar alınırken hata oluştu');
      } finally {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedTurId) {
      toast.error('Lütfen ruhsat türünü seçin.');
      return;
    }

    if (!selectedActivityId) {
      toast.error('Lütfen faaliyet konusunu seçin.');
      return;
    }

    if (!verilisTarihi) {
      toast.error('Lütfen veriliş tarihini girin.');
      return;
    }

    if (isGSM && selectedClassId) {
      const emptyWarehouses = Object.entries(warehouseForm).filter(([_, val]) => !val.trim());
      if (emptyWarehouses.length > 0) {
        toast.error('Lütfen tüm depo bilgilerini doldurun.');
        return;
      }
    }
    ...
    try {
      ...
      const response = await fetch(`${CONFIG.apiUrl}/Ruhsat/add`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`
        },
                    ...
                    <Grid container spacing={2}>
                      {warehouses.map(w => (
                        <Grid item xs={12} sm={6} md={4} key={w.id}>
                          <TextField
                            fullWidth
                            label={`${w.name} için bilgi`}
                            value={warehouseForm[w.id] || ''}
                            onChange={(e) => handleWarehouseChange(w.id, e.target.value)}
                          />
                        </Grid>
                      ))}
                    </Grid>

...
export function NumberingForm({ data, id }) {
  const NUMARATAJ = {
    OZEL_ISYERI: 0,
    RESMI_KURUM: 1,
    YENI_BINA: 2,
  ...
  useEffect(() => {
    ...
    setCaddeSokak(data.caddeSokak || '');
    setDisKapi(data.disKapi || '');
    setIcKapiNo(data.icKapiNo || '');
    setSiteAdi(data.siteAdi || '');
    setEskiAdres(data.eskiAdres || '');
    setBlokAdi(data.blokAdi || '');
    setAdresNo(data.adresNo || '');
    setIsYeriUnvani(data.isYeriUnvani || '');
    setAda(data.ada || '');
    setParsel(data.parsel || '');
    setNumaratajType(data.numaratajType ?? 0);
    setMahalleId(data.mahalleId || '');
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${CONFIG.apiUrl}/Numarataj/update`, {
        method: 'POST',
        headers: {

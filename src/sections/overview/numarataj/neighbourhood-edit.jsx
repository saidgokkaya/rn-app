...
export function NeighbourhoodEditForm({ currentSchema }) {
  const defaultValues = {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const response = await fetch(`${CONFIG.apiUrl}/Numarataj/add-mahalles`, {
        method: 'POST',
        headers: {
      ...
      setTimeout(() => {
        window.location.href = '/dashboard/numbering/common-areas';
      }, 1000);
    } catch (error) {
      toast.error('Lütfen tüm alanları doldurup tekrar deneyeniz!');
    }
